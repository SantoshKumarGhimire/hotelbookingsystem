		...
		<form:form modelAttribute="room" method="POST" action="addroom"
			...
			<table>
				...
				<tr>
					<td>Features</td>
					<td>
						<%-- 					<form:checkboxes itemValue="${roomFeatures}" itemLabel="${roomFeatures}" --%>
						<%-- 							path="roomFeatures" items="${roomFeatures}" /> --%> 
						
						
						<c:forEach
							items="${roomFeatures}" var="current">
							<form:checkbox itemValue="current" itemLabel="current"
						 							path="roomFeatures" items="current" />
						</c:forEach>
